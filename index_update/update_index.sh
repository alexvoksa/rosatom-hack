...
export DATABASE_URL=postgres://hackathon:hackathon@hackathon_db:5432/hackathon

apt-get update
apt-get install postgresql-client -y

psql "$DATABASE_URL" <<-EOSQL > data.txt
SELECT '["' || string_agg(table_name, '", "') || '"]' as data
  FROM information_schema.tables
 WHERE table_schema='public'
   AND table_type='BASE TABLE';
EOSQL

TABLES="$(cat data.txt | egrep -v '(^-)' | egrep -v 'data' | egrep -v '(1 row)' | tr -d '[:space:]')"
export TABLES

echo "$TABLES"

cat <<EOF > sql2meili.json
{
  "database":
  {"adapter": "postgresql",
    "host": "hackathon_db",
...
cat <<EOF > modified_meili_loader.py
"""
...
class MeilisearchConn:
    ...
    def upload_data_to_index(self, index: str, data: List[Dict],
        ...
        """
        self.meilisearch_client.index(index).add_documents(
...
def export_tables(db_con: DatabaseConn, meili: MeilisearchConn,
    ...
    for idx, table_name in enumerate(db_con.tables):
        ...
        if total_size < max_chunk_size:
            values: List = []
            for row in db_con.database_engine.execute(db_con.metadata.tables[table_name].select()):
                new_row = dict(row)
                for new_row_key in new_row.keys():
                   if isinstance(new_row[new_row_key], Decimal):
                       new_row[new_row_key] = float(new_row[new_row_key])
                   if isinstance(new_row[new_row_key], (datetime, date)):
                       new_row[new_row_key] = new_row[new_row_key].isoformat()
            ...
            for chunk in range(chunks):
                values: List = []
                for row in db_con.database_engine.execute(select(["*"]).select_from(db_con.metadata.tables[table_name]).order_by(primary_key_name).offset(chunk * max_chunk_size).limit(max_chunk_size)):
                    new_row = dict(row)
                    for new_row_key in new_row.keys():
                       if isinstance(new_row[new_row_key], Decimal):
                           new_row[new_row_key] = float(new_row[new_row_key])
...
rm -rf sql2meili/
git clone "https://github.com/Kamefrede/sql2meili.git"

cp sql2meili.json sql2meili/
cp modified_meili_loader.py sql2meili/

cd sql2meili/ || echo "NO DIR" exit

pip3 install cython SQLAlchemy==1.3.22 jsonschema==3.2.0 meilisearch psycopg2-binary==2.8.6
python3.9 ./modified_meili_loader.py --config "$(pwd)/sql2meili.json" --chunk_size 5000

# python3.9 ./sql2meili.py --config "$(pwd)/sql2meili.json" --chunk_size 10000