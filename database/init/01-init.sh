...
psql -v ON_ERROR_STOP=1 -U "$PGUSER" -d "$PGDATABASE" <<-EOSQL
  ...
  ON CONFLICT DO NOTHING
  ;
  \copy innovate_suppliers(inn, name, phone, email, description, reputation, successful_tenders, unsuccessful_tenders) FROM '../constant_files/innovative.csv' DELIMITER ',' CSV HEADER
  COMMIT;
EOSQL